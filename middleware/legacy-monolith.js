...
const CORS_ORIGINS = String(process.env.CORS_ORIGINS || process.env.FRONTEND_URL || 'http://localhost')
    .split(',')
...
const normalizeAuthRole = (role) => {
    ...
    if (r === 'system_admin' || r === 'systemadmin' || r === 'system_administrator' || r === 'sysadmin') {
        return 'system_admin';
    }
    if (r === 'department_admin' || r === 'departmentadmin' || r === 'dept_admin' || r === 'deptadmin' || r === 'department' || r === 'chairperson' || r === 'department_head' || r === 'admin') {
...
const recordSecurityEvent = async (req, eventType, severity, attemptedIdentity, details) => {
    try {
        await dbWrite.query(
        ...
        logger.warn(`[SecurityEvent] Could not persist ${eventType}: ${error.message}`);
    }
};
...
const authenticateJWT = (req, res, next) => {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        jwt.verify(token, jwtKey, async (err, user) => {
            if (err) {
                await recordSecurityEvent(req, 'INVALID_ACCESS_TOKEN', 'MEDIUM', null, 'An invalid or expired bearer token was rejected.');
                return res.status(403).json({ error: "Invalid or expired token." });
            }
            try {
                const username = user?.username || user?.email;
                const databaseRole = await getDatabaseRole(username);
                const tokenRole = normalizeAuthRole(user?.dbRole || user?.role);
                if (!databaseRole || databaseRole !== tokenRole) {
                    await recordSecurityEvent(req, 'REVOKED_ACCOUNT_TOKEN', 'HIGH', username, 'A token for an inactive, renamed, or role-changed account was rejected.');
                    return res.status(403).json({ error: 'This account is inactive, changed, or no longer authorized.' });
                }
                req.user = { ...user, username, dbRole: databaseRole };
                next();
            } catch (validationError) {
                logger.error('[Auth] Active account validation failed:', validationError.message);
...
const getDatabaseRole = async (username) => {
    if (!username) return '';
    const result = await dbRead.query(
        'SELECT role FROM users WHERE LOWER(email) = LOWER($1) AND LOWER(status) = \'approved\' AND is_active = TRUE LIMIT 1',
        [username]
    );
    return result.rows.length ? normalizeAuthRole(result.rows[0].role) : '';
};
...
app.post('/api/login', loginLimiter, async (req, res) => {
    try {
        ...
        if (userRecord.status.toLowerCase() !== 'approved' || userRecord.is_active === false) { // Generic error for client
            await recordSecurityEvent(req, 'INACTIVE_ACCOUNT_LOGIN', 'HIGH', userRecord.email, 'A login was attempted for an inactive or unapproved account.');
        ...
        if (!validPassword) {
            await recordSecurityEvent(req, 'FAILED_LOGIN', 'MEDIUM', userRecord.email, 'Login failed because the password did not match.');
            return res.status(401).json({ error: "Invalid email or password" });
        }
...
app.post('/api/fabric/register-user', authenticateJWT, requireRegistrarOrInternal, async (req, res) => {
    try {
        ...
        const existingIdentity = await wallet.get(email);
        if (existingIdentity) {
            if (existingIdentity.mspId !== mspId) {
                return res.status(409).json({ error: 'An identity with this email exists in a different Fabric organization.' });
            }
            return res.status(200).json({ status: 'Success', message: 'Blockchain wallet already exists.' });
        }
        ...
        try {
            ...
            if (regErr.toString().includes('code: 20') || regErr.toString().includes('Authentication failure')) {
                ...
                adminUser = await provider.getUserContext(newAdminIdentity, 'admin');
                await registerUser(adminUser);
            } else if (regErr.toString().toLowerCase().includes('already registered')) {
                logger.warn(`[RegisterUser] ${email} is already registered with the CA; attempting enrollment.`);
            } else {
                throw regErr;
            }
        }
...
app.post('/api/forgot-password', passwordResetLimiter, async (req, res) => {
    try {
        ...
        const user = await dbRead.query('SELECT * FROM Users WHERE LOWER(email) = LOWER($1)', [email]);
        ...
        logger.info('[PasswordReset] A password reset link was generated and queued for delivery.');
...
app.get('/api/student-transactions', authenticateJWT, async (req, res) => {
    let username;
    try {
        username = getCallerIdentity(req);
        const role = req.user
            ? normalizeAuthRole(req.user.dbRole || req.user.role)
            : await getDatabaseRole(username);
        if (role !== 'student') {
            return res.status(403).json({ error: 'Student transaction history is available only to the authenticated student.' });
        }

        const { contract } = await getContractForUser(username, 'student');
        const result = await contract.evaluateTransaction('GetStudentTransactions');
        return res.status(200).json({ status: 'success', data: JSON.parse(result.toString()) });
    } catch (error) {
        await clearCacheOnError(username, error);
        return res.status(500).json({ error: process.env.NODE_ENV === 'production' ? 'Unable to retrieve transaction history' : error.message });
    }
});

app.get('/api/grade-history/:id', authenticateJWT, async (req, res) => {
    let username;
    try {
        username = getCallerIdentity(req);
        const role = req.user
            ? normalizeAuthRole(req.user.dbRole || req.user.role)
            : await getDatabaseRole(username);
        if (!['faculty', 'department_admin', 'registrar'].includes(role)) {
            return res.status(403).json({ error: 'Grade audit history is restricted to authorized academic staff.' });
        }

        const { contract } = await getContractForUser(username, role);
        const currentResult = await contract.evaluateTransaction('ReadGrade', req.params.id);
        const currentRecord = JSON.parse(currentResult.toString());
        if (role === 'faculty' && String(currentRecord.faculty_id || '').toLowerCase() !== String(username).toLowerCase()) {
            return res.status(403).json({ error: 'Faculty may view history only for grades they submitted.' });
        }
        if (role === 'department_admin') {
            const profile = await dbRead.query(
                `SELECT ap.department, p.program_code, p.program_name
                 FROM users u JOIN adminprofiles ap ON ap.user_id = u.id
                 LEFT JOIN academic_programs p ON LOWER(p.program_name) = LOWER(ap.department) OR LOWER(p.program_code) = LOWER(ap.department)
                 WHERE LOWER(u.email) = LOWER($1) LIMIT 1`,
                [username]
            );
            const scope = profile.rows[0];
            const recordProgram = String(currentRecord.program || currentRecord.course || '').toLowerCase();
            const allowedPrograms = [scope?.department, scope?.program_code, scope?.program_name]
                .filter(Boolean).map(value => String(value).toLowerCase());
            if (!allowedPrograms.includes(recordProgram)) {
                return res.status(403).json({ error: 'Chairpersons may view history only for their assigned academic program.' });
            }
        }
        const result = await contract.evaluateTransaction('GetGradeHistory', req.params.id);
        return res.status(200).json({ status: 'success', data: JSON.parse(result.toString()) });
    } catch (error) {
        await clearCacheOnError(username, error);
        return res.status(500).json({ error: process.env.NODE_ENV === 'production' ? 'Unable to retrieve grade history' : error.message });
    }
});

app.post('/api/fabric/audit-event', authenticateJWT, requireRegistrarOrInternal, async (req, res) => {
    let username;
    try {
        username = getCallerIdentity(req);
        const role = await getDatabaseRole(username);
        if (role !== 'registrar') {
            return res.status(403).json({ error: 'A registered Registrar ledger identity is required.' });
        }
        const { contract } = await getContractForUser(username, 'registrar');
        const result = await contract.submitTransaction('CreateAuditEvent', JSON.stringify(req.body));
        return res.status(201).json({ status: 'success', data: JSON.parse(result.toString()) });
    } catch (error) {
        await clearCacheOnError(username, error);
        return res.status(500).json({ error: process.env.NODE_ENV === 'production' ? 'Unable to record audit event' : error.message });
    }
});
...
app.get('/api/get-grade/:id', authenticateJWT, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            if (callerRole !== 'student' && (req.isInternal || walletErr.message.includes('not found'))) {
                const { contract } = await getContractForUser('system-admin-registrar', 'registrar');
                contractToUse = contract;
        ...
        try {
            ...
            if (callerRole !== 'student' && (evalErr.message.includes('creator is malformed') || evalErr.message.includes('access denied') || evalErr.message.includes('UNKNOWN'))) {
        ...
        if (callerRole === 'student' && String(record.student_hash || '').toLowerCase() !== String(username).toLowerCase()) {
...
app.post('/api/update-grade', authenticateJWT, authorizeRole(['faculty', 'department_admin', 'registrar']), async (req, res) => {
    let username;
    try {
        username = getCallerIdentity(req);
        const { contract } = await getContractForUser(username, req.user ? req.user.dbRole : null);
...
app.post('/api/approve-grade/:id', authenticateJWT, authorizeRole(['department_admin', 'registrar']), async (req, res) => {
    let username;
    try {
        username = getCallerIdentity(req);
        const { contract } = await getContractForUser(username, req.user ? req.user.dbRole : null);
...
app.post('/api/finalize-grade/:id', authenticateJWT, authorizeRole(['registrar']), async (req, res) => {
    let username;
    try {
        username = getCallerIdentity(req);
        const { contract } = await getContractForUser(username, req.user ? req.user.dbRole : null);
...
app.post('/api/return-grade/:id', authenticateJWT, authorizeRole(['department_admin', 'registrar']), async (req, res) => {
    let username;
    try {
        username = getCallerIdentity(req);
        const { note } = req.body;
...
app.post('/api/batch-issue-grade', async (req, res) => {
    ...
    try {
        ...
        username = req.headers['x-user-identity'];
        if (!username) {
            return res.status(400).json({ 
                error: 'Missing faculty identity',
                hint: 'The internal caller must provide the authenticated account in x-user-identity.'
            });
        }

        const databaseRole = await getDatabaseRole(username);
        if (!['faculty', 'department_admin'].includes(databaseRole)) {
            return res.status(403).json({ error: 'The authenticated account is not permitted to issue grades.' });
        }
        ...
        const { contract } = await getContractForUser(username, databaseRole);
...
app.get('/api/bootstrap', requireInternalKey, async (req, res) => { // This should be moved to a K8s Job
    try {
        ...
        try {
            ...
            const provider = wallet.getProviderRegistry().getProvider(adminIdentity.type);
            const adminUser = await provider.getUserContext(adminIdentity, 'admin');
            await ca.register({ enrollmentID: registrarEmail, enrollmentSecret: registrarPass, role: 'admin', attrs: [{ name: 'role', value: 'registrar', ecert: true }] }, adminUser);
...
async function startServer() {
    await initializeFabricArtifacts();
...
startServer().catch(error => {
    logger.fatal({ error }, 'Middleware failed to start');
    process.exit(1);
});
