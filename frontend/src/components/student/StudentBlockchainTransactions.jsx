              ...
              {transactions.map((transaction, index) => {
                ...
                return (
                  <tr key={`${transaction.transactionId}-${index}`} className="align-top hover:bg-slate-50">
                    <td className="whitespace-nowrap px-4 py-3">{transaction.timestamp ? new Date(transaction.timestamp).toLocaleString() : '—'}</td>
                    ...
                    <td className="px-4 py-3">{transaction.professor || 'Not recorded'}</td><td className="whitespace-nowrap px-4 py-3"><span className="block">{transaction.semester || '—'}</span><span className="text-xs text-slate-500">{transaction.schoolYear || '—'}</span></td><td className="px-4 py-3 capitalize">{transaction.term || '—'}</td><td className="px-4 py-3 font-bold">{transaction.grade || '—'}</td>
