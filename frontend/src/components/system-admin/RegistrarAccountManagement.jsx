...
import { createRegistrarAccount, fetchRegistrarAccounts, resetManagedAccountPassword, updateRegistrarAccount } from '../../services/api';
...
const RegistrarAccountManagement = () => {
  ...
  const toggleActive = async (account) => {
    ...
    setSaving(true); setNotice(null);
    try {
      const response = await updateRegistrarAccount(account.id, { isActive: !account.isActive });
      setNotice({ type: response?.data?.warning ? 'warning' : 'success', message: response?.data?.warning || `Registrar ${account.isActive ? 'deactivated' : 'activated'}.` });
      await load();
    } catch (error) { setNotice({ type: 'error', message: error.message }); }
    finally { setSaving(false); }
  };
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="text-xl font-bold text-[#003366]">Create Registrar Account</h2><p className="mb-5 mt-1 text-sm text-slate-500">The Registrar role is fixed and cannot be removed when credentials change.</p>
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4 flex justify-between"><div><h2 className="text-xl font-bold text-[#003366]">Registrar Access</h2><p className="text-sm text-slate-500">System Administrators can update Registrar email, activation, and passwords only.</p></div><button onClick={load} className="rounded-lg bg-slate-100 px-3 py-2 text-sm font-semibold">Refresh</button></div>
      {loading ? <p className="py-8 text-center text-slate-500">Loading accounts…</p> : <div className="space-y-4">{accounts.map((account) => {
        ...
        return <div key={account.id} className="rounded-xl border border-slate-200 p-4">
          <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-bold text-slate-800">{account.fullName}</p><p className="text-xs text-slate-500">{account.accountId} · Registrar</p></div><button disabled={saving} onClick={() => toggleActive(account)} className={`rounded-lg px-3 py-2 text-sm font-bold ${account.isActive ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-800'}`}>{account.isActive ? 'Deactivate' : 'Activate'}</button></div>
